...
app = FastAPI(title="NU Masters Results API", version="1.6.0")
...
def _value(soup: BeautifulSoup, *labels: str) -> str | None:
    """Extract a labelled value without relying on Bootstrap presentation classes."""
...
def _get_form(client: requests.Session) -> tuple[str, str, str] | None:
    """Fetch the official Masters form and keep its cookies in the same session."""
    headers = {**HTML_HEADERS, "Referer": f"{NU_ORIGIN}/"}
    last_error: requests.RequestException | None = None
    urls = (MASTERS_URL, f"{MASTERS_URL}?_={int(time.time())}")
    for url in urls:
        try:
...
def _captcha(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        csrf, action, question = _get_form(client) or (None, None, None)
        if not csrf or not action or not question:
...
__all__ = [
    "MASTERS_CAPTCHA_REFRESH_URL",
    "MASTERS_EXAMINATIONS",
    "MASTERS_URL",
    "EXAMINATION_METRICS",
    "app",
    "parse_masters_result",
]
