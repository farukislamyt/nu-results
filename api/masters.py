...
app = FastAPI(title="NU Masters Results API", version="1.1.1")
...
def _captcha_response(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "session": seal({"csrf": token_el.get("value", ""), "cookies": client.cookies.get_dict(), "action": action, "module": "masters"}),
            "expires_in": 300,
        }
    except RuntimeError:
...
def masters_result(body: dict[str, Any], request: Request):
    ...
    try:
        ...
        response = client.post(
            str(action),
            data={"_token": csrf, "examination_name": examination_name, "year": str(body.get("year", "")), "examination_roll": str(body.get("examination_roll", "")), "registration_no": str(body.get("registration_no", "")), "captcha": str(body.get("captcha", ""))},
            timeout=28,
